...
- **Confirmation flag:** `is_confirmed` is an **agent workflow** toggle—not authorization, not cryptographic proof of human approval, and **not a substitute for OS-level approval**. Writes, moves, deletes, and any copy request carrying `overwrite=True` require it by default so the model explicitly opts in. The copy gate follows the overwrite flag even when the destination is currently absent, avoiding a check-then-copy race silently replacing a newly created destination.
